...
experimentalDisplays.water_height_grad	= new VectorFieldDisplay( { 
	getField: function (world) {
		var water_height = ScalarField.max_scalar(world.displacement, world.SEALEVEL);
		var gradient = ScalarField.gradient(water_height);
		return gradient;
		var sediment_flux_metric = VectorField.mult_scalar_field(gradient, world.unsubductable_sediment);
		var divergence = VectorField.divergence(gradient);
		ScalarField.sub_scalar(divergence, Float32Dataset.average(divergence), divergence);
	}
} );
experimentalDisplays.height_div 	= new ScalarHeatDisplay( { min: '-3e11', max: '3e11',  
	getField: function (world) {
		var water_height = ScalarField.max_scalar(world.displacement, world.SEALEVEL);
		var gradient = ScalarField.gradient(water_height);
		var sediment_flux_metric = VectorField.mult_scalar_field(gradient, world.unsubductable_sediment);
		var divergence = VectorField.divergence(sediment_flux_metric);
		ScalarField.sub_scalar(divergence, Float32Dataset.average(divergence), divergence);
		return divergence;
	}
} );
experimentalDisplays.sediment 	= new ScalarHeatDisplay( { min: '0.', max: '30.',  
